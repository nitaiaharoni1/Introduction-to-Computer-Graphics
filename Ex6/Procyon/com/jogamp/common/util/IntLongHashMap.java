...
public class IntLongHashMap implements Cloneable, Iterable<Entry>
{
    ...
    public IntLongHashMap(final int n) {
        this(n, 0.75f);
    }
    
    public IntLongHashMap(final int n, final float loadFactor) {
    ...
    private IntLongHashMap(final float loadFactor, final int n, final int size, final int mask, final int capacity, final int threshold, final long keyNotFoundValue) {
        this.keyNotFoundValue = -1L;
        this.loadFactor = loadFactor;
    ...
    public Object clone() {
        final IntLongHashMap intLongHashMap = new IntLongHashMap(this.loadFactor, this.table.length, this.size, this.mask, this.capacity, this.threshold, this.keyNotFoundValue);
        ...
        for (int i = this.table.length - 1; i >= 0; --i) {
            ...
            for (int j = size - 1; j >= 0; --j) {
                ...
                if (IntLongHashMap.isPrimitive) {
                    entry = new Entry(entry2.key, entry2.value, entry);
                }
                else {
                    entry = (Entry)ReflectionUtil.createInstance(IntLongHashMap.entryConstructor, entry2.key, ReflectionUtil.callMethod(entry2.value, getCloneMethod(entry2.value), new Object[0]), entry);
                }
            }
            intLongHashMap.table[i] = entry;
        }
    ...
    public boolean containsValue(final long n) {
        ...
        while (length-- > 0) {
            for (Entry next = table[length]; next != null; next = next.next) {
                ...
                else if (ReflectionUtil.callMethod(n, IntLongHashMap.equalsMethod, next.value)) {
                    return true;
                }
            }
        }
    ...
    public long put(final int n, final long value) {
        final Entry[] table = this.table;
    ...
    public void putAll(final IntLongHashMap intLongHashMap) {
        for (final Entry entry : intLongHashMap) {
