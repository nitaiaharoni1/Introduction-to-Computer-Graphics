...
public class LongLongHashMap implements Cloneable, Iterable<Entry>
{
    ...
    public LongLongHashMap(final int n) {
        this(n, 0.75f);
    }
    
    public LongLongHashMap(final int n, final float loadFactor) {
    ...
    private LongLongHashMap(final float loadFactor, final int n, final int size, final int mask, final int capacity, final int threshold, final long keyNotFoundValue) {
        this.keyNotFoundValue = -1L;
        this.loadFactor = loadFactor;
    ...
    public Object clone() {
        final LongLongHashMap longLongHashMap = new LongLongHashMap(this.loadFactor, this.table.length, this.size, this.mask, this.capacity, this.threshold, this.keyNotFoundValue);
        ...
        for (int i = this.table.length - 1; i >= 0; --i) {
            ...
            for (int j = size - 1; j >= 0; --j) {
                ...
                if (LongLongHashMap.isPrimitive) {
                    entry = new Entry(entry2.key, entry2.value, entry);
                }
                else {
                    entry = (Entry)ReflectionUtil.createInstance(LongLongHashMap.entryConstructor, entry2.key, ReflectionUtil.callMethod(entry2.value, getCloneMethod(entry2.value), new Object[0]), entry);
                }
            }
            longLongHashMap.table[i] = entry;
        }
    ...
    public boolean containsValue(final long n) {
        ...
        while (length-- > 0) {
            for (Entry next = table[length]; next != null; next = next.next) {
                ...
                else if (ReflectionUtil.callMethod(n, LongLongHashMap.equalsMethod, next.value)) {
                    return true;
                }
            }
        }
    ...
    public void putAll(final LongLongHashMap longLongHashMap) {
        for (final Entry entry : longLongHashMap) {
            this.put(entry.key, entry.value);
        }
    }
    
    public long remove(final long n) {
        final Entry[] table = this.table;
        final int n2 = HashUtil.getAddrHash32_EqualDist(n) & this.mask;
        Entry entry2;
        Entry next;
    ...
    public long setKeyNotFoundValue(final long keyNotFoundValue) {
        final long keyNotFoundValue2 = this.keyNotFoundValue;
        this.keyNotFoundValue = keyNotFoundValue;
    ...
    static {
        ...
        if (!(isPrimitive = type.isPrimitive())) {
            final EntryCM entryCM = AccessController.doPrivileged((PrivilegedAction<EntryCM>)new PrivilegedAction<EntryCM>() {
                final /* synthetic */ Class val$keyClazz = type2;
                final /* synthetic */ Class val$valueClazz = type;
    ...
    public static final class Entry
    {
        ...
        Entry(final long key, final long value, final Entry next) {
            this.key = key;
            this.value = value;
            this.next = next;
        }
