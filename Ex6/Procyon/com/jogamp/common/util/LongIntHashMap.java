...
public class LongIntHashMap implements Cloneable, Iterable<Entry>
{
    ...
    public LongIntHashMap(final int n) {
        this(n, 0.75f);
    }
    
    public LongIntHashMap(final int n, final float loadFactor) {
        this.keyNotFoundValue = -1;
        if (n > 1073741824) {
    ...
    private LongIntHashMap(final float loadFactor, final int n, final int size, final int mask, final int capacity, final int threshold, final int keyNotFoundValue) {
        this.keyNotFoundValue = -1;
        this.loadFactor = loadFactor;
    ...
    public Object clone() {
        final LongIntHashMap longIntHashMap = new LongIntHashMap(this.loadFactor, this.table.length, this.size, this.mask, this.capacity, this.threshold, this.keyNotFoundValue);
        ...
        for (int i = this.table.length - 1; i >= 0; --i) {
            ...
            for (int j = size - 1; j >= 0; --j) {
                ...
                if (LongIntHashMap.isPrimitive) {
                    entry = new Entry(entry2.key, entry2.value, entry);
                }
                else {
                    entry = (Entry)ReflectionUtil.createInstance(LongIntHashMap.entryConstructor, entry2.key, ReflectionUtil.callMethod(entry2.value, getCloneMethod(entry2.value), new Object[0]), entry);
                }
            }
            longIntHashMap.table[i] = entry;
        }
    ...
    public boolean containsValue(final int n) {
        ...
        while (length-- > 0) {
            for (Entry next = table[length]; next != null; next = next.next) {
                ...
                else if (ReflectionUtil.callMethod(n, LongIntHashMap.equalsMethod, next.value)) {
                    return true;
                }
            }
        }
    ...
    public int put(final long n, final int value) {
        final Entry[] table = this.table;
    ...
    public void putAll(final LongIntHashMap longIntHashMap) {
        for (final Entry entry : longIntHashMap) {
    ...
    public int setKeyNotFoundValue(final int keyNotFoundValue) {
        final int keyNotFoundValue2 = this.keyNotFoundValue;
        this.keyNotFoundValue = keyNotFoundValue;
    ...
    public static final class Entry
    {
        ...
        Entry(final long key, final int value, final Entry next) {
            this.key = key;
            this.value = value;
            this.next = next;
        }
