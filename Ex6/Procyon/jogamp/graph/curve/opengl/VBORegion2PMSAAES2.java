...
public class VBORegion2PMSAAES2 extends GLRegion
{
    ...
    public VBORegion2PMSAAES2(final int n, final TextureSequence textureSequence, final int n2) {
    ...
    protected void drawImpl(final GL2ES2 gl2ES2, final RegionRenderer regionRenderer, final int[] array) {
        if (0 >= this.indicesBuffer.getElementCount()) {
            if (VBORegion2PMSAAES2.DEBUG_INSTANCE) {
        ...
        if (width <= 0 || height <= 0 || null == array || array[0] <= 0) {
            ...
            if (abs || abs2 || this.fboDirty || this.isShapeDirty() || null == this.fbo || this.fbo.getNumSamples() != array[0]) {
                final float n13 = this.box.getMinX() - n9;
                final float n14 = this.box.getMinY() - n10;
                final float n15 = this.box.getMaxX() + n9 + n7;
                final float n16 = this.box.getMaxY() + n10 + n8;
                this.gca_FboVerticesAttr.seal(false);
                final FloatBuffer floatBuffer = (FloatBuffer)this.gca_FboVerticesAttr.getBuffer();
                floatBuffer.put(0, n13);
                floatBuffer.put(1, n14);
                floatBuffer.put(3, n13);
                floatBuffer.put(4, n16);
                floatBuffer.put(6, n15);
                floatBuffer.put(7, n16);
                floatBuffer.put(9, n15);
                floatBuffer.put(10, n14);
                floatBuffer.position(12);
                this.gca_FboVerticesAttr.seal(true);
                FloatUtil.makeOrtho(this.pmvMatrix02, 0, true, n13, n15, n14, n16, -1.0f, 1.0f);
                this.useShaderProgram(gl2ES2, regionRenderer, this.getRenderModes(), true, this.getQuality(), array[0]);
                this.renderRegion2FBO(gl2ES2, renderState, n11, n12, width, height, array);
            }
            ...
            this.renderFBO(gl2ES2, renderState, width, height, array[0]);
        }
    }
    
    private void renderFBO(final GL2ES2 gl2ES2, final RenderState renderState, final int n, final int n2, final int n3) {
        ...
        this.fbo.use(gl2ES2, this.fbo.getSamplingSink().getTextureAttachment());
    ...
    private void renderRegion2FBO(final GL2ES2 gl2ES2, final RenderState renderState, final int n, final int n2, final int n3, final int n4, final int[] array) {
        if (0 >= n || 0 >= n2) {
        ...
        if (null == this.fbo) {
            this.fboWidth = n;
            this.fboHeight = n2;
            (this.fbo = new FBObject()).init(gl2ES2, this.fboWidth, this.fboHeight, array[0]);
            array[0] = this.fbo.getNumSamples();
            this.fbo.attachColorbuffer(gl2ES2, 0, true);
            if (!hintMaskSet) {
                this.fbo.attachRenderbuffer(gl2ES2, FBObject.Attachment.Type.DEPTH, 0);
            }
            final FBObject samplingSink = new FBObject();
            samplingSink.init(gl2ES2, this.fboWidth, this.fboHeight, 0);
            samplingSink.attachTexture2D(gl2ES2, 0, true, 9728, 9728, 33071, 33071);
            if (!hintMaskSet) {
                samplingSink.attachRenderbuffer(gl2ES2, FBObject.Attachment.Type.DEPTH, 0);
            }
            this.fbo.setSamplingSink(samplingSink);
            this.fbo.resetSamplingSink(gl2ES2);
        }
        else if (n != this.fboWidth || n2 != this.fboHeight || this.fbo.getNumSamples() != array[0]) {
            this.fbo.reset(gl2ES2, n, n2, array[0]);
            array[0] = this.fbo.getNumSamples();
            this.fboWidth = n;
            this.fboHeight = n2;
        }
